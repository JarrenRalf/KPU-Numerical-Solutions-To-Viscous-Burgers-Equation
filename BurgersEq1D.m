function [u, x, t, denominator] = BurgersEq1D(f, c, L, T, dx, dt, N)
...
%
%         f  = The initial condition passed as an anonymous function
...
t = 0.001:dt:T; % t-grid points
x = 0.001:dx:L; % x-grid points
x = x';         % transpose to make column vector

a0 = integral(@(x) f(x), 0, L)/L; % Calculate the a0 constant
...
for n = 1:N
    ...
    a(n) = 2*integral(@(x) f(x).*cos(n*pi*x/L), 0, L)/L;
